...
def process_resume_and_match_jobs(pdf_file):
    ...
    Returns:
    - dict: Dictionary containing extracted text and matched jobs.
    """
    ...
    if not extracted_text.strip():
        return {"extracted_text": "No relevant text extracted from the resume.", "matched_jobs": []}
    ...
    # Step 3: Match the resume text with job descriptions
    try:
        vectorizer = TfidfVectorizer(stop_words='english')
        all_documents = [extracted_text] + job_descriptions
    ...
    return {"extracted_text": extracted_text, "matched_jobs": matched_jobs}
